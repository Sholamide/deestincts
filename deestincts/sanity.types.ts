...
// Query: *[_type == "project" && defined(slug.current) && isFeatured == true] | order(_updatedAt desc) {      _id,  title,  "slug": slug.current,  excerpt,  featuredImage,  projectType,  isFeatured,  client,  }
export type FeaturedProjectsQueryResult = Array<never>;
// Variable: projectQuery
// Query: *[_type == "project" && defined(slug.current)] | order(_updatedAt desc) {      _id,  title,  "slug": slug.current,  excerpt,  featuredImage,  projectType,  isFeatured,  client,  }
export type ProjectQueryResult = Array<never>;
...
// Variable: AboutQuery
// Query: *[_type == "about"][0] {    content[]{    ...,    markDefs[]{      ...,        _type == "link" => {    "page": page->slug.current,    "post": post->slug.current  }    }  },     _id, title, slug, content, teamMembers[] { name, role, bio, image }  }
export type AboutQueryResult = null;
// Variable: AllPostsQuery
// Query: *[_type == "post"] {    content[]{    ...,    markDefs[]{      ...,        _type == "link" => {    "page": page->slug.current,    "post": post->slug.current  }    }  },      _id,  "status": select(_originalId in path("drafts.**") => "draft", "published"),  "title": coalesce(title, "Untitled"),  "slug": slug.current,  excerpt,  coverImage,  "date": coalesce(date, _updatedAt),  "author": author->{firstName, lastName, picture},  }
export type AllPostsQueryResult_2 = Array<{
  content: Array<{
    children?: Array<{
      marks?: Array<string>;
      text?: string;
